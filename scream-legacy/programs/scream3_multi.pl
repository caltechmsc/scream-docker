...
# Variables
my $ff           = "/app/lib/ff/dreiding-0.4-x6.par";
my $selections   = 10;
my $rotlib       = 0.5;
...
# No A/G/P
@finreslist = ();
foreach my $r (@reslist) {
    if ($r !~ /^[AGP]/) { push @finreslist, $r; }
}
@reslist = @finreslist;
if (@reslist < 1) {
    die "scream3 :: No residues specified for SCREAMing!\n";
...
# Run scream
$command = "$screammulti $bgf $rotlib $ff $selections " . join(" ", @reslist);
`$command >& scream.out`;

# Success
if (-e "best_1.bgf") {
    my @glob = "best_*.bgf";
    foreach my $g (@glob) { `/app/lib/perl/BGFFormat.pl -b $g`; }
...
unlink "Field1.bgf";
unlink glob "Residue-E-best_*.txt";
...
Usage:
 :: scream3_multi.pl -b {bgf} -l {rotlib} -n {selections} -r {residues} -c {charge}
...
Input:
 ...
 :: -n | --selections  :: Integer
 :: Number of conformations to output.  Default = 10
 ...
 :: Default: /app/lib/ff/dreiding-0.4-x6.par
...
Description:
 ...
 :: IMPORTANT: Do not use this program for mutations!
 :: IMPORTANT: Do not use this program for more than 10 residues
 :: IMPORTANT: A/G/P will not be SCREAMed
